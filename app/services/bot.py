...
class BotService:
    ...
    def _manage_positions(self, positions: list) -> None:
        for pos in positions:
            try:
                tick = self.gateway.tick(pos.symbol)
                if tick is None:
                    continue
                price = tick.bid if pos.type == 0 else tick.ask
                rates = self.gateway.rates(pos.symbol, TIMEFRAME_M15, 50)
                ...
                info = mt5.symbol_info(pos.symbol) if mt5 else None
    ...
    def _evaluate_symbol(self, canonical: str, broker_symbol: str, bot_positions: list) -> None:
        ...
        print(f"{canonical} | closed={closed_dt.strftime('%Y-%m-%d %H:%M')} | forming={forming_dt.strftime('%Y-%m-%d %H:%M')}", flush=True)
        ...
        try:
            ...
            print(f"Database: UNAVAILABLE ({_db_exc}) ? submitting to MT5 without journal", flush=True)
            db = None
        try:
